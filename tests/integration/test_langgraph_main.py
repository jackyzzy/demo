...
def test_langgraph_main():
    """测试LangGraph main.py"""
    print("🧪 测试LangGraph Agent main.py")
    print("=" * 60)
...
if http_models:
    ...
    try:
        with redirect_stdout(null_output), redirect_stderr(null_output):
            agent = LangGraphAgent(model_key=http_models[0])
    ...
    try:
        with redirect_stdout(null_output), redirect_stderr(null_output):
            agent = LangGraphAgent(model_key=first_model)
        print("✅ Agent创建成功")
        print("✅ 输出抑制正常")
...
'''
    ...
    try:
        # 在langgraph环境中运行测试
        conda_python = os.path.expanduser("~/anaconda3/envs/langgraph-agent/bin/python")
        ...
        if result.returncode == 0:
            print("✅ LangGraph Agent测试通过")
            print(result.stdout)
        else:
            print("❌ LangGraph Agent测试失败")
...
def main():
    """主测试函数"""
    print("🚀 LangGraph Agent 修复验证测试")
    print("=" * 80)
    
    tests = [
        ("中文输入支持", test_chinese_input),
        ("LangGraph Agent", test_langgraph_main)
    ]
    ...
    if passed == len(results):
        print("🎉 所有测试通过！LangGraph Agent修复成功。")
    else:
