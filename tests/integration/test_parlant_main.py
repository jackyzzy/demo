...
def test_parlant_main():
    """测试Parlant main.py"""
    print("🧪 测试Parlant Agent main.py")
    print("=" * 60)

    # 测试脚本内容
    test_script = '''
import sys
sys.path.append("/home/zzy/code/comp_agent/parlant-agent/src")
...
if http_models:
    ...
    try:
        with redirect_stdout(null_output), redirect_stderr(null_output):
            agent = ParlantAgent(model_key=http_models[0])
    ...
    try:
        with redirect_stdout(null_output), redirect_stderr(null_output):
            agent = ParlantAgent(model_key=first_model)
        print("✅ Agent创建成功")
        print("✅ 输出抑制正常")
...
'''
    ...
    try:
        # 在parlant环境中运行测试
        conda_python = os.path.expanduser("~/anaconda3/envs/parlant-agent/bin/python")
        if os.path.exists(conda_python):
            ...
            result = subprocess.run(
                ["bash", "-c", f"source ~/anaconda3/etc/profile.d/conda.sh && conda activate parlant-agent && PYTHONPATH=/home/zzy/code/comp_agent python -c \"{test_script}\""],
                capture_output=True,
                text=True,
                cwd="/home/zzy/code/comp_agent"
            )
        ...
        if result.returncode == 0:
            print("✅ Parlant Agent测试通过")
            print(result.stdout)
        else:
            print("❌ Parlant Agent测试失败")
...
def show_usage_instructions():
    ...
    print("1. 激活conda环境:")
    print("   conda activate parlant-agent")
    print("   cd parlant-agent")
    ...
    print("   ✅ 优化用户界面体验")
    print("   ✅ Parlant异步架构")
    print("   ✅ Guidelines系统集成")


def main():
    """主测试函数"""
    print("🚀 Parlant Agent 实现验证测试")
    print("=" * 80)

    tests = [
        ("中文输入支持", test_chinese_input),
        ("Parlant Agent", test_parlant_main)
    ]
    ...
    if passed == len(results):
        print("🎉 所有测试通过！Parlant Agent实现成功。")
    else:
